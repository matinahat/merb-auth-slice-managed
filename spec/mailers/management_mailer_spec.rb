...
describe "ManagementMailer" do

  before(:all) do
    Merb::Router.prepare { add_slice(:merb_auth_slice_managed)}
    User.auto_migrate!
  end
  ...
  describe MerbAuthSliceManaged::ManagementMailer do

    def deliver(action, mail_opts= {},opts = {})
      MerbAuthSliceManaged::ManagementMailer.dispatch_and_deliver action, mail_opts, opts
    ...
    it "should mention the activation link in the signup emails" do
      deliver(:signup, @mailer_params, :user => @u)
      the_url = MerbAuthSliceManaged::ManagementMailer.new.slice_url(:activate, :activation_code => @u.activation_code)
